#########################################################################################
###                          Run FFP on EML Tower 2016-2017                        ###
###                                Code by HGR 2/2021                                 ###
#########################################################################################
...
### Load data ###########################################################################
final.16 <- read.csv('/scratch/hgr7/flux_tower_footprint/data_input/ffp_ready_2016.csv',
                     na.strings = '-999')
final.16 <- data.table(final.16)
#########################################################################################
...
# # run the function on 2016
# Define how many cores you want to use
UseCores <- 28

# split up data
data.rows <- nrow(final.16)
...
for (i in 1:UseCores) {
  ...
  if (i < UseCores) {
    ...
    end.row <- nrow(final.16)
  }
  final.16.list[[i]] <- final.16[start.row:end.row]
}

cl <- makeCluster(UseCores, outfile = '/scratch/hgr7/flux_tower_footprint/ffp_raw_output/log.txt')
registerDoParallel(cl)

ffp.df.2016 <- foreach(i=1:UseCores, .combine = rbind) %dopar% {
  # load libraries
  library(sf)
  library(raster)
  library(tidyverse)
  ...
  # calculate ffp
  calc.ffp.loop(final.16.list[[i]], ec_sf, karst.mtopo.brick, seq(10, 90, 10))
  
}

stopCluster(cl)

write.csv(ffp.df.2016,
          '/scratch/hgr7/flux_tower_footprint/ffp_2016.csv',
          row.names = FALSE)
#########################################################################################