#########################################################################################
###                          Run FFP on EML Tower 2018-2019                        ###
###                                Code by HGR 2/2021                                 ###
#########################################################################################
...
### Load data ###########################################################################
final.18 <- read.csv('/scratch/hgr7/flux_tower_footprint/data_input/ffp_ready_2018.csv',
                     na.strings = '-999')
final.18 <- data.table(final.18)
#########################################################################################
...
# # run the function on 2018
# Define how many cores you want to use
UseCores <- 28

# split up data
data.rows <- nrow(final.18)
...
for (i in 1:UseCores) {
  ...
  if (i < UseCores) {
    ...
    end.row <- nrow(final.18)
  }
  final.18.list[[i]] <- final.18[start.row:end.row]
}

cl <- makeCluster(UseCores, outfile = '/scratch/hgr7/flux_tower_footprint/ffp_raw_output/log18.txt')
registerDoParallel(cl)

ffp.df.2018 <- foreach(i=1:UseCores, .combine = rbind) %dopar% {
  # load libraries
  library(sf)
  library(raster)
  library(tidyverse)
  ...
  # calculate ffp
  calc.ffp.loop(final.18.list[[i]], ec_sf, karst.mtopo.brick, seq(10, 90, 10))
  
}

stopCluster(cl)

write.csv(ffp.df.2018,
          '/scratch/hgr7/flux_tower_footprint/ffp_2018.csv',
          row.names = FALSE)
#########################################################################################