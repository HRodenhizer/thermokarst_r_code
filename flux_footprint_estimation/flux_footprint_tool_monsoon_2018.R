#########################################################################################
###                          Run FFP on EML Tower 2019-2020                        ###
###                                Code by HGR 2/2021                                 ###
#########################################################################################
...
### Load data ###########################################################################
final.19 <- read.csv('/scratch/hgr7/flux_tower_footprint/data_input/ffp_ready_2019.csv',
                     na.strings = '-999')
final.19 <- data.table(final.19)
#########################################################################################
...
# # run the function on 2019
# Define how many cores you want to use
UseCores <- 28

# split up data
data.rows <- nrow(final.19)
...
for (i in 1:UseCores) {
  ...
  if (i < UseCores) {
    ...
    end.row <- nrow(final.19)
  }
  final.19.list[[i]] <- final.19[start.row:end.row]
}

cl <- makeCluster(UseCores, outfile = '/scratch/hgr7/flux_tower_footprint/ffp_raw_output/log.txt')
registerDoParallel(cl)

ffp.df.2019 <- foreach(i=1:UseCores, .combine = rbind) %dopar% {
  # load libraries
  library(sf)
  library(raster)
  library(tidyverse)
  ...
  # calculate ffp
  calc.ffp.loop(final.19.list[[i]], ec_sf, karst.mtopo.brick, seq(10, 90, 10))
  
}

stopCluster(cl)

write.csv(ffp.df.2019,
          '/scratch/hgr7/flux_tower_footprint/ffp_2019.csv',
          row.names = FALSE)
#########################################################################################